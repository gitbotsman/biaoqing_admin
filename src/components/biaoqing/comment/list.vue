          ...
          <tr v-for="(comment,index) in comments.items">
            <td class="replay-container">
              <div class="flex flex-center">
                <div>作品ID:<router-link class="" :to="'/subjectdetail/'+comment.subjectId">{{comment.subjectId}}</router-link></div>
                <router-link class="comment-user-tx" :to="'/userdetail/'+comment.user.id" >
                  <img :src="comment.user.fullAvatar" />
                </router-link>
                <div class="comment-user-info">

                  <router-link class="name" :class="{'text-success':(comment.user.userType=='5')}" :to="'/userdetail/'+comment.user.id">{{comment.user.name}}</router-link>
...
<script>
  ...
  import { Subject,Comment } from '../../../resources'
